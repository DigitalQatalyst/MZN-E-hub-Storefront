...
import { useParams } from 'react-router-dom'; // Keep useParams for now, but we'll replace it below
...
import Link from 'next/link'; // Next.js Link instead of react-router-dom Link
...
interface MarketplaceDetailsPageProps {
  ...
  params?: { itemId: string }; // Add params prop for Next.js dynamic routes
}
...
const MarketplaceDetailsPage: React.FC<MarketplaceDetailsPageProps> = ({
  ...
  // Use params from props instead of useParams hook
  const { itemId } = params || {};
  
  // Use Next.js router instead of React Router's useNavigate
  const router = useRouter();
  const searchParams = useSearchParams(); // This works in Next.js 15
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (finalItemData) {
          ...
          const timer = setTimeout(() => {
            router.push(config.route); // Use Next.js router.push instead of navigate
          }, 5000);
    ...
    fetchItemDetails();
  }, [itemId, marketplaceType, bookmarkedItems, shouldTakeAction, router, config]);
            ...
            <ol className="inline-flex items-center space-x-1 md:space-x-2">
              ...
              <li>
                <div className="flex items-center">
                  ...
                  <Link href={config.route} className="ml-1 text-gray-600 hover:text-gray-900 md:ml-2">
    ...
    return <div className="min-h-screen flex flex-col bg-gray-50">
        ...
        <div className="container mx-auto px-4 py-8 flex items-center justify-center min-h-[300px] flex-grow">
          <div className="text-center">
            ...
            <Link href={config.route} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors inline-block">
  ...
  const SummaryCard = ({
    isFloating = false
  }: { isFloating?: boolean }) => <div ref={isFloating ? null : summaryCardRef} className={`
        ...
        <section className="bg-gray-50 py-10 border-t border-gray-200">
          <div className="container mx-auto px-4 md:px-6 max-w-7xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <Link href={config.route} className="text-blue-600 font-medium hover:text-blue-800 flex items-center">
            ...
            {relatedItems.length > 0 ? <div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {relatedItems.map(relatedItem => <div key={relatedItem.id} className="bg-white rounded-lg shadow p-4 cursor-pointer hover:shadow-md transition-shadow" onClick={() => router.push(`/marketplace/${marketplaceType}/${relatedItem.id}`)}>
                      <div className="flex items-center mb-3">
