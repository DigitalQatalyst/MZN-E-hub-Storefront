...
export default function NonFinancialServiceCatalogue({
  ...
  useEffect(() => {
    ...
    if (category) {
      // Map URL category to the corresponding filter key
      const categoryMap: { [key: string]: CategoryFilterKeys } = {
        'events-networking': 'eventsAndNetworking',
        'partnerships-opportunities': 'partnershipsAndOpportunities',
        'academy-training': 'academyAndTraining',
        'operational-advisory': 'operationalAdvisory',
        'proximity-incubators': 'proximityIncubators',
        'incentives-listing': 'incentivesListing',
        'digital-solutions': 'digitalSolutions',
        'export-trade-facilitation': 'exportAndTradeFacilitation',
        'legal-compliance-licensing': 'legalComplianceAndLicensing'
      };

      const filterKey = categoryMap[category as CategoryCodes];
      if (filterKey) {
        // Set the corresponding category filter
        setCategoriesFilters(prev => ({
          ...Object.fromEntries(
            Object.keys(prev).map(key => [key, key === filterKey])
          )
        } as { [key in CategoryFilterKeys]: boolean }));
      }
    }
  }, [searchParams]);
